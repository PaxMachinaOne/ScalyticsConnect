...
*   **Interactive Documentation:** Explore the API interactively using the [Swagger UI documentation](https://github.com/scalytics/MCPServer/blob/main/Connect/swagger.json) available on Github.
...
1.  **API Enabled:** An administrator must have globally enabled the Scalytics API feature in the Admin Dashboard. If disabled, all requests will fail.
2.  **API Key:** You need to generate a personal Scalytics API key.
...
6.  **Important:** Your API key (starting with `sk-scalytics-`) will be displayed **only once**. Copy this key immediately and store it securely. You will not be able to retrieve it again.
7.  You can view your generated keys (by name) and delete them from the API Keys settings page.
...
The entire `/v1/chat/completions` endpoint is subject to a **global rate limit** configured by the administrator (e.g., a maximum number of requests per minute for the whole service). This limit is shared across all users and API keys. If the global limit is exceeded, you will receive a `429 Too Many Requests` error. Check the `RateLimit-*` response headers (if included by the server configuration) for details on the global limits.
...
*   **403 Forbidden Error:** If you receive a `403 Forbidden` error (potentially directly from Caddy), it might be due to the User-Agent header in your request. The server configuration may block requests from default User-Agents associated with tools like `curl`, `wget`, or basic Python HTTP clients.
    *   **Solution:** Set a custom `User-Agent` header in your request. For example, using `curl`:
        ```bash
        curl ... -H "User-Agent: MyCustomClient/1.0" ...
        ```
        Or for programmatic clients (like Python `requests`):
        ```python
        import requests
        ...
        headers = {
            'Authorization': 'Bearer YOUR_SCALYTICS_API_KEY',
            'Content-Type': 'application/json',
            'User-Agent': 'MyCustomClient/1.0' # Add this
        }
        # ... rest of your request setup
        response = requests.post(url, headers=headers, json=payload)
        ```
        Using a descriptive User-Agent (e.g., `SCAPI/1.0`, `MyIntegration/2.1`) is recommended. Standard OpenAI library User-Agents should also be allowed.
