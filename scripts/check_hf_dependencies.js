...
function safeExec(command, args = [], options = {}) {
  try {
    execFileSync(command, args, { stdio: options.silent ? 'ignore' : 'inherit' });
    return true;
  } catch (error) {
    if (!options.silent) {
      console.log(`Warning: Command failed: ${command} ${args.join(' ')}`);
...
function safeChmod(filePath) {
  ...
  try {
    ...
    console.log(`Warning: Could not make ${path.basename(filePath)} executable: ${error.message}`);
    console.log('Warning: The script may not work correctly without executable permissions');
    return false;
  }
}
...
async function checkPython() {
  try {
    ...
    const pythonVersion = execFileSync('python3', ['--version']).toString();
    console.log(`Python detected: ${pythonVersion.trim()}`);
    return true;
  } catch (error) {
    console.log('Warning: Python not found or not working correctly');
    console.log('Warning: Python features will be disabled');
    return false;
  }
}
  ...
  try {
    ...
    if (!fs.existsSync(path.dirname(venvPath))) {
      try {
        ...
        console.log(`Warning: Could not create directory for virtual environment: ${mkdirError.message}`);
        return false;
      }
    }
    ...
    try {
      ...
      execFileSync('python3', ['-m', 'venv', '--without-pip', venvPath]);
      ...
      console.log('Warning: Could not create isolated virtual environment with venv:', venvError.message);
      ...
      try {
        ...
        console.log('Warning: Could not create environment with standard venv:', stdVenvError.message);
      }
    }
    ...
    try {
      ...
      execFileSync('python3', ['-m', 'virtualenv', venvPath]);
      console.log('Virtual environment created using virtualenv');
      ensureVenvIsolation(venvPath);
      return true;
    } catch (virtualenvError) {
      console.log('Warning: Could not create virtual environment using virtualenv:', virtualenvError.message);
    }

    console.log('Warning: Failed to create Python virtual environment');
    return false;
  } catch (error) {
    console.log('Failed to set up virtual environment:', error.message);
    console.log('Warning: Will proceed without a virtual environment. Some features may not work correctly.');
    return false;
  }
}
...
function ensureVenvIsolation(venvPath) {
  try {
    // Read pyvenv.cfg directly (avoid check-then-act race condition)
    const pyvenvCfgPath = path.join(venvPath, 'pyvenv.cfg');
    let content;
    try {
      content = fs.readFileSync(pyvenvCfgPath, 'utf8');
    } catch (readErr) {
      if (readErr.code === 'ENOENT') {
        console.log('Warning: pyvenv.cfg not found, cannot ensure isolation');
        return false;
      }
      throw readErr;
    }
    ...
    console.log(`Warning: Error ensuring virtual environment isolation: ${error.message}`);
    return false;
  }
}
...
function bootstrapPip(venvPath) {
  try {
    ...
    if (!fs.existsSync(pythonPath)) {
      console.log('Warning: Python executable not found in virtual environment');
      return false;
    }
    ...
    try {
      execFileSync('curl', ['-s', 'https://bootstrap.pypa.io/get-pip.py', '-o', getPipPath]);
    } catch (curlError) {
      try {
        execFileSync('wget', ['-q', 'https://bootstrap.pypa.io/get-pip.py', '-O', getPipPath]);
      } catch (wgetError) {
        console.log('Warning: Failed to download get-pip.py. Curl and wget both failed.');
        return false;
      }
    }
    ...
    execFileSync(pythonPath, [getPipPath, '--no-user', '--isolated', '--no-cache-dir'], { env });
    ...
    if (fs.existsSync(pipPath)) {
      ...
      console.log('Warning: Pip installation failed');
      return false;
    }
  } catch (error) {
    console.log(`Warning: Error bootstrapping pip: ${error.message}`);
    return false;
  }
}
...
function configurePipEnvironment(venvPath) {
  // Create pip.conf directory (recursive: true is safe if it already exists)
  const pipConfigDir = path.join(venvPath, 'pip');
  try {
    fs.mkdirSync(pipConfigDir, { recursive: true });
  } catch (error) {
    console.log(`Warning: Could not create pip config directory: ${error.message}`);
  }

  // Write pip.conf using exclusive creation flag to avoid race condition
...
`;
    fs.writeFileSync(pipConfigFile, pipConfigContent, { flag: 'wx' });
    console.log(`Created pip.conf at ${pipConfigFile}`);
  } catch (error) {
    // EEXIST means file already exists, which is fine
    if (error.code !== 'EEXIST') {
      console.log(`Warning: Could not create pip.conf: ${error.message}`);
    }
  }
...
async function installDependencies() {
  ...
  try {
    ...
    if (!fs.existsSync(venvPath)) {
      console.log('Warning: Virtual environment not found, skipping dependency installation');
      return false;
    }
    ...
    if (!fs.existsSync(pipPath)) {
      console.log('Warning: pip not found in virtual environment, skipping dependency installation');
      return false;
    }
    ...
    const installPackage = (pkg) => {
      try {
        ...
        execFileSync(pipBin, ['install', '--no-user', '--isolated', '--no-cache-dir', pkg], { env });
        console.log(`Installed ${pkg}`);
        return true;
      } catch (error) {
        console.log(`Warning: Failed to install ${pkg}: ${error.message}`);
        return false;
      }
    };
    ...
    try {
      ...
      if (fs.existsSync(pipPath)) {
        ...
        const pipBin = path.join(venvBinDir, 'pip');
        execFileSync(pipBin, ['install', '--no-user', '--isolated', '--no-cache-dir', 'huggingface_hub'], { env });
        console.log('Installed huggingface_hub');
      } else {
        console.log('Warning: Pip not found, skipping huggingface_hub installation');
      }
    } catch (hfError) {
      console.log('Warning: Failed to install huggingface_hub, some features may be limited');
    }

    console.log('Basic dependencies installed successfully');
    return true;
  } catch (error) {
    console.log('Warning: Dependency installation failed:', error.message);
    console.log('Warning: Application will continue with limited functionality');
    return false;
  }
}
...
function createModelsDir() {
  ...
  if (!fs.existsSync(modelsDir)) {
    try {
      ...
      console.log('Warning: Could not create models directory:', error.message);
    }
  } else {
...
async function checkSSLSupport() {
  try {
    const { stdout } = await execFilePromise('python3', ['-c', 'import ssl; print("SSL available")']);
    if (stdout.includes('SSL available')) {
      console.log('SSL support available in Python');
      return true;
    }
  } catch (error) {
    try {
      const { stdout } = await execFilePromise('python', ['-c', 'import ssl; print("SSL available")']);
      if (stdout.includes('SSL available')) {
        console.log('SSL support available in Python');
        return true;
      }
    } catch (error2) {
      console.log('Warning: SSL module not available in Python');
      return false;
    }
  }
...
async function checkHuggingFaceDependencies() {
  ...
  if (!pythonOk) {
    console.log('Warning: Python not available. Model management will be limited.');
    console.log('Warning: Application will continue with basic functionality only.');
    return false;
  }

  // Check SSL support - this affects how we download models
  await checkSSLSupport();
  ...
  if (venvOk) {
    ...
    console.log('Warning: Virtual environment setup failed, skipping dependency installation');
  }
