  ...
  try {
    ...
    if (reasoningModelInfoFull) {
        ...
        if (!isRequestedModelAccessible) {
            console.warn('[DeepSearchAPI:%s] Requested reasoning model \'%s\' (ID: %s) found but is not accessible to user %s. Attempting fallback to local model.', apiTaskId, originalReasoningModelName, reasoningModelInfoFull.id, userId);
            performReasoningFallback = true;
        }
    } else {
        if (originalReasoningModelName.toLowerCase() !== 'local') {
            console.warn("[DeepSearchAPI:%s] Requested reasoning model '%s' not found. Attempting fallback to local model.", apiTaskId, originalReasoningModelName);
            performReasoningFallback = true;
        }
    }
    ...
    if (synthesisModelInfoFull) {
        ...
        if (!isRequestedModelAccessible) {
            console.warn('[DeepSearchAPI:%s] Requested synthesis model \'%s\' (ID: %s) found but is not accessible to user %s. Attempting fallback to local model.', apiTaskId, originalSynthesisModelName, synthesisModelInfoFull.id, userId);
            performSynthesisFallback = true;
        }
    } else {
        if (originalSynthesisModelName.toLowerCase() !== 'local') {
            console.warn("[DeepSearchAPI:%s] Requested synthesis model '%s' not found. Attempting fallback to local model.", apiTaskId, originalSynthesisModelName);
            performSynthesisFallback = true;
        }
    }
        ...
        try {
            ...
            if (apiKeyData?.key) {
                ...
                if (provider.serviceName === 'Google Search' && apiKeyData.extra_config) {
                    ...
                    } catch (e) { /* console.warn('[DeepSearchAPI:%s] Error parsing extra_config for Google Search key: %s', apiTaskId, e.message); */ }
                }
            } else {
                console.warn('[DeepSearchAPI:%s] API key NOT found for search provider: %s for user %s', apiTaskId, provider.serviceName, userId);
            }
        } catch (err) { /* console.warn('[DeepSearchAPI:%s] Error fetching API key for search provider %s: %s', apiTaskId, provider.serviceName, err.message); */ }
    }
    ...
    console.error('[LiveSearchAPI:%s] Failed to initiate live search or stream:', apiTaskId, error);
    if (!res.headersSent) {
