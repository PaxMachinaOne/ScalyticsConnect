...
if os.getenv("PYTHON_DEEP_SEARCH_SCRAPE_CONCURRENCY"):
    try:
        ...
        logging.info(f"[Config] Warning: Invalid value for PYTHON_DEEP_SEARCH_SCRAPE_CONCURRENCY env var. Using default: {settings.PYTHON_DEEP_SEARCH_SCRAPE_CONCURRENCY}", file=sys.stderr)

logging.info(f"[Config] Effective DEEP_SEARCH_SCRAPE_CONCURRENCY: {settings.DEEP_SEARCH_SCRAPE_CONCURRENCY}", file=sys.stderr)
