        ...
        **kwargs: Any,
        ...
        for prompt_text in prompts:
            ...
            temp_model_info = {
                ...
                "temperature": self.model_info.get("temperature", self.settings.DEEP_SEARCH_REASONING_DEFAULT_TEMP),
            }
...
class LLMReasoning:
    ...
    def get_session(self) -> aiohttp.ClientSession:
        """Get the shared aiohttp ClientSession, creating it if it doesn't exist."""
        if self.http_session is None or self.http_session.closed:
            # Nuanced timeout: 5 mins total, but 10s between data chunks to detect stalls.
...
"""
    ...
    async def _call_xai_directly_async(
        ...
        try:
            ...
            if response_format_type == "json_object":
                try:
                    # Use the main sanitization function
                    cleaned_output = self._sanitize_llm_json_output(output_content, log_prefix)
    ...
    def _sanitize_llm_json_output(self, json_str: str, log_prefix: str) -> str:
        """
        ...
        try:
            ...
            # Use a stack to find the matching closing bracket/brace
            stack = []
            end_index = -1
    ...
    async def _execute_llm_call(
        ...
        cache_key_parts = [
            ...
            str(model_info.get("temperature", self.settings.DEEP_SEARCH_REASONING_DEFAULT_TEMP)),
        ...
        common_llm_params = { 
            "temperature": model_info.get("temperature", self.settings.DEEP_SEARCH_REASONING_DEFAULT_TEMP),
        }
        ...
        if provider_name == 'xai':
            ...
            for attempt in range(max_retries + 1):
                ...
                xai_result = await self._call_xai_directly_async(
                    ...
                    max_tokens=None # Pass max_tokens as None
                )
            ...
            # This is the new direct-to-vLLM logic
            vllm_base_url = api_config.get("llm_local_apiBaseUrl")
            ...
            payload = {
                "model": model_info.get("served_model_name"), # CORRECTED: Use served_model_name
            ...
            error_message_for_return = "Max retries reached for direct vLLM API call."
                    ...
                    async with session.post(direct_vllm_api_url, json=payload, headers=headers) as response:
                        if response.status != 200:
                            ...
                            if response.status in [500, 502, 503, 504] and attempt < max_retries:
                                ...
                                error_message_for_return = f"Direct vLLM API Error {response.status}: {error_text}"
                                break
                        
                        # SUCCESS: Process streaming response for HTTP 200
                        buffer = ""
                        ...
                        async for chunk_bytes in response.content.iter_chunked(8192):
                            if is_cancelled_flag and is_cancelled_flag.is_set():
                                ...
                                raise asyncio.CancelledError("vLLM API stream cancelled")
            ...
            if expected_output_format == "json":
                try:
                    cleaned_json_str = self._sanitize_llm_json_output(llm_response_content, log_prefix)
            ...
            # This is the existing litellm logic for all other providers (Google, Mistral, etc.)
            model_arg_for_litellm = model_name
            api_key = None
            api_base_url = None
            ...
            for attempt in range(max_retries + 1):
                ...
                try:
                    ...
                    if expected_output_format == "json":
                        try:
                            cleaned_json_str = self._sanitize_llm_json_output(raw_content, log_prefix)
                            parsed_json = json.loads(cleaned_json_str)
                            if not isinstance(parsed_json, (dict, list)):
                                raise json.JSONDecodeError("Parsed JSON is not a dict or list", cleaned_json_str, 0)
...
2.  Do NOT add any text, explanations, or commentary before or after the JSON object.
3.  ALL string values, especially those within lists, MUST be enclosed in double quotes.
4.  Any double quotes that are part of the string content itself MUST be escaped with a backslash (e.g., "a string with \\"internal quotes\\" like this").
...
"""
    ...
    async def synthesize_initial_draft(
        ...
        processing_language: Optional[str] = "en",
        is_cancelled_flag: Optional[asyncio.Event] = None 
    ) -> Dict[str, Any]:
    ...
    async def synthesize_with_vector_focus(
        ...
        if processing_language and processing_language != "en":
            language_instruction = f"\n- **LANGUAGE REQUIREMENT:** You MUST write this entire report in {processing_language}. Do not use English."
...
Synthesize a comprehensive and high-quality report of approximately {self.settings.DEEP_SEARCH_SYNTHESIS_TARGET_WORD_COUNT} words based on ALL the provided information.
...
- Maintain a professional, objective, and authoritative tone.{language_instruction}
...
Output the complete, revised Final Report (aim for approximately {self.settings.DEEP_SEARCH_REFINEMENT_TARGET_WORD_COUNT} words):"""
    ...
    async def summarize_text(
        ...
        prompt = f"{context_header}Provide a concise summary of the following text, aiming for {self.settings.DEEP_SEARCH_SUMMARY_TARGET_SENTENCE_COUNT}. Focus on the main points and key information presented in this specific excerpt. Base your summary ONLY on the text provided below:\n\n---\n{text_to_summarize}\n---\n\nConcise Summary ({self.settings.DEEP_SEARCH_SUMMARY_TARGET_SENTENCE_COUNT}):"
...
"""
    ...
    async def rerank_chunks_for_relevance(
        ...
        batch_size = self.settings.DEEP_SEARCH_RERANK_LLM_BATCH_SIZE
...
Based on your general knowledge, please provide a comprehensive, well-structured initial answer to the user's question above, aiming for approximately {self.settings.DEEP_SEARCH_PREFLIGHT_TARGET_WORD_COUNT} words.
...
Comprehensive Initial Answer (approx. {self.settings.DEEP_SEARCH_PREFLIGHT_TARGET_WORD_COUNT} words):"""
...
"""
            ...
            prompt = f"""Given the following query or research step, please write a concise, factual paragraph (approximately {self.settings.DEEP_SEARCH_HYDE_TARGET_WORD_COUNT}) that directly and ideally answers it.
...
Ideal Hypothetical Paragraph (approx. {self.settings.DEEP_SEARCH_HYDE_TARGET_WORD_COUNT}):
"""
...
Text Chunk:
---
{chunk_text[:self.settings.DEEP_SEARCH_ISOLATED_SUMMARY_MAX_INPUT_CHARS]}
---
...
"""
    ...
    async def translate_text_async(
        ...
        """
        Translates text to a target language using an LLM.
        """
        if is_cancelled_flag and is_cancelled_flag.is_set():
            return {"translated_text": None, "usage": {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0}, "error": "Operation cancelled."}

        if not text_to_translate:
            return {"translated_text": "", "usage": {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0}}
...
"""
        ...
        translation_model_info = model_info.copy()
        translation_model_info["temperature"] = 0.1

        result = await self._execute_llm_call(
            request_type="translation",
            prompt=prompt,
            model_info=translation_model_info,
            api_config=api_config,
            user_id_for_internal_call=user_id,
            request_id=request_id,
            expected_output_format="text",
    ...
    async def translate_texts_batch_async(
        self,
        ...
        """
        Translates multiple texts to a target language using an LLM in a single batch call.
        Returns a list of translated texts in the same order as input.
        """
        if is_cancelled_flag and is_cancelled_flag.is_set():
            return {"translated_texts": [], "usage": {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0}, "error": "Operation cancelled."}

        if not texts_to_translate:
            return {"translated_texts": [], "usage": {"prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0}}

        # Format all texts into a single batch prompt
        formatted_texts = ""
        for i, text in enumerate(texts_to_translate):
            formatted_texts += f"TEXT_{i+1}:\n{text}\n---NEXT_TEXT---\n\n"

        prompt = f"""Translate the following {len(texts_to_translate)} texts into {target_language}.
For each text, output ONLY the translation followed by '---TRANSLATION_SEPARATOR---' on a new line.
Maintain the exact same order as the input texts.

Texts to Translate:
{formatted_texts}

Instructions:
- Translate each text accurately
- After each translation, add '---TRANSLATION_SEPARATOR---' on a new line
- Do not include the original text numbers or any other formatting
...
"""
        ...
        batch_response = result.get("output", "")
        if not batch_response:
            return {"translated_texts": [], "usage": result.get("usage", {}), "error": "Empty batch translation response"}

        # Split by separator and clean up
        translations = batch_response.split("---TRANSLATION_SEPARATOR---")
        cleaned_translations = [t.strip() for t in translations if t.strip()]
        ...
        return {"translated_texts": cleaned_translations[:len(texts_to_translate)], "usage": result.get("usage", {})}
