...
// Import Middleware
const { globalFeatureToggle } = require('../middleware/globalFeatureToggleMiddleware');
const { authScalyticsApi } = require('../middleware/authScalyticsApiMiddleware');
const { rateLimiterMiddleware } = require('../middleware/rateLimitMiddleware');
 ...
 *         # session_id: # Optional, might be auto-generated or passed by client
 *         #   type: string
...
router.get(
  ...
  scalyticsApiController.listModelsHandler
);
 ...
 *           application/json:
 *             schema:
 *               # Add schema for non-streaming response (similar to OpenAI)
 *               type: object 
 *               properties:
 *                 id: { type: string }
 ...
 *                 model: { type: string }
 *                 choices: { type: array, items: { $ref: '#/components/schemas/ChatMessage' } } # Simplified
 *                 usage: { type: object, properties: { prompt_tokens: {type: integer}, completion_tokens: {type: integer}, total_tokens: {type: integer} } }
 *           text/event-stream:
 *              schema:
 *                # Add schema for streaming response chunks (similar to OpenAI)
 *                type: object 
 *                properties:
 *                  id: { type: string }
 ...
 *                  created: { type: integer }
 *                  model: { type: string }
 *                  choices: { type: array, items: { type: object, properties: { delta: { $ref: '#/components/schemas/ChatMessage' }, finish_reason: { type: string, nullable: true } } } }
 *                  usage: { type: object, nullable: true, properties: { prompt_tokens: {type: integer}, completion_tokens: {type: integer}, total_tokens: {type: integer} } }
 *       '400':
...
router.post(
  ...
  authScalyticsApi,
  rateLimiterMiddleware,
  scalyticsApiController.handleChatCompletion
);
...
router.post(
  ...
  authScalyticsApi,
  rateLimiterMiddleware,
  vectorApiController.embedTextsHandler
);
...
router.post(
  ...
  authScalyticsApi,
  rateLimiterMiddleware,
  vectorApiController.addDocumentsHandler // Assuming this handler exists
);
...
router.post(
  ...
  authScalyticsApi,
  rateLimiterMiddleware,
  vectorApiController.searchVectorsHandler // Assuming this handler exists
);
...
router.post(
  ...
  authScalyticsApi,
  rateLimiterMiddleware,
  scalyticsApiController.generateImageApi // Corrected to use scalyticsApiController
);
...
router.use((req, res) => {
  res.status(404).json({
    success: false,
    error: {
      message: `The requested endpoint ${req.method} ${req.originalUrl} does not exist on the Scalytics API v1.`,
