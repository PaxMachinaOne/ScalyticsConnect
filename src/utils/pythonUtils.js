 ...
 * @param {Object} options - Options to pass to child_process.execFile
...
async function executePythonScript(scriptPath, args = [], options = {}) {
  ...
  if (wrapperExists) {
    try {
      ...
      console.log('[WARN] Python exec command: %s %s %s', PYTHON_WRAPPER, scriptPath, args.join(' '));

      return await execFilePromise(PYTHON_WRAPPER, [scriptPath, ...args], execOptions);
    } catch (error) {
  ...
  if (pythonEnv.exists) {
    try {
      console.log('[WARN] Using detected Python virtual environment: %s %s', pythonEnv.python, scriptPath);
      ...
      return await execFilePromise(pythonEnv.python, [scriptPath, ...args], execOptions);
    } catch (error) {
  ...
  try {
    return await execFilePromise('python3', [scriptPath, ...args], options);
  } catch (error) {
...
async function checkPythonEnvironment() {
  try {
    ...
    const { stdout, stderr } = await execFilePromise(PYTHON_WRAPPER, ['-c', 'print("Python environment ok")']);
