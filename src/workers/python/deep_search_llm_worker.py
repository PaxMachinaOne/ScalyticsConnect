...
"""
Deep Search LLM Worker

Handles the Reasoning and Synthesis steps of the Deep Search process
...
class DeepSearchLLMWorker:
    """
    Worker class for handling LLM calls via litellm for Deep Search.
    """
    def __init__(self):
    ...
    def cleanup(self):
        ...
        print("Deep Search LLM worker cleaning up.", file=sys.stderr)

    def handle_shutdown_signal(self, signum, frame):
        """Handle termination signals."""
        if not self.shutdown_requested:
            print(f"Received signal {signum}, shutting down Deep Search LLM worker...", file=sys.stderr)
    ...
    def run(self):
        ...
        print("Deep Search LLM worker ready.", file=sys.stderr)
        print("Deep Search LLM worker entering main loop...", file=sys.stderr)
        try:
            ...
            if 'loop' in locals() and loop.is_running():
                loop.close()
            print("Deep Search LLM worker finished.", file=sys.stderr)
